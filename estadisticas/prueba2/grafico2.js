...
// INICIALIZACIÓN
function iniciar2() {
    return cargarDatos(archivo2) // Cargar los datos del JSON
        .then(data2 => {
            ...
            if (window.chart2) {
                try { window.chart2.destroy(); } catch (e) { }
            }

            const contenedor = document.querySelector("#grafico2");
            if (contenedor) contenedor.innerHTML = "";

            const chart = crearGrafico2(categories_M_2, values_M_2, categories_V_2, values_V_2);
            chart.render();
            window.chart2 = chart;
